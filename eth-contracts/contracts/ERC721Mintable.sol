...
contract Pausable is Ownable {
    ...
    function set(bool status) public onlyOwner {
        if (status != _paused) {
            _paused = status;
            if (status == true) emit Unpaused(msg.sender);
            if (status == false) emit Paused(msg.sender);
        }
    }
}
...
contract ERC721 is Pausable, ERC165 {
    ...
    function approve(address to, uint256 tokenId) public {
        ...
        require(to != ownerOf(tokenId), 'Given address is owner of tokenId');

        // TODO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(msg.sender == get() || isApprovedForAll(msg.sender, to), 'Error in approve');
    ...
    function getApproved(uint256 tokenId) public view returns (address) {
        ...
        if (_tokenApprovals[tokenId] != address(0)) return _tokenApprovals[tokenId];
    }
     ...
     */
    // tokenId has owner, therefore it exists
    ...
    function _mint(address to, uint256 tokenId) internal onlyOwner {
        ...
        require(ownerOf(tokenId) == address(0) && to == address(to), 'tokenId already exists or given address it invalid');
  
        // TODO mint tokenId to given address & increase token count of owner
        uint ownerTokenCount = _ownedTokensCount[to].current();
        _tokenOwner[ownerTokenCount] = to;
        _ownedTokensCount[to].increment();

        // TODO emit Transfer event
        emit Transfer(get(), to, tokenId);
    }
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(from == ownerOf(tokenId), 'Caller is not tokenId owner');
...
contract ERC721Metadata is ERC721Enumerable, usingOraclize {
    ...
    function setTokenURI(uint tokenId) internal {
        require(_exists(tokenId), 'Something went wrong');
...
contract CustomERC721Token is ERC721Metadata('CustomERC721Token Name', 'CET', 'https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/') {
